...
def signup2():
    ...
    if len(check_result) == 0:
        # 사용 가능한 아이디
        mydb.execute_query(querys.signup_query, input_id, input_password, input_name,inplace = True)
        return redirect('/')
    else: 
...
def dashboard() :
    if 'user_info' not in session :
        return redirect('/')
    # 유저가 보낸 데이터를 변수에 저장
    # get 방시긍로 보내온 데이터는 request.args에 존재 
    # 종목코드
